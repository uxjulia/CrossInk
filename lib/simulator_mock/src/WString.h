...
public:
    ...
    int read() const { if (readPos < s.length()) return s[readPos++]; return -1; }
    bool endsWith(const String& suffix) const { return s.size() >= suffix.s.size() && s.substr(s.size() - suffix.s.size()) == suffix.s; }
    bool endsWith(const char* suffix) const { return suffix && s.size() >= strlen(suffix) && s.substr(s.size() - strlen(suffix)) == suffix; }
