...
static inline int mbedtls_base64_decode(unsigned char* dst, size_t dlen, size_t* olen,
    ...
    *olen = 0;
    for (i = 0; i + 3 < slen; i += 4) {
        ...
        dst[n++] = (c0 << 2) | (c1 >> 4);
        if (src[i+2] != '=') { if ((signed char)c2 < 0) return MBEDTLS_ERR_BASE64_INVALID_CHARACTER; dst[n++] = (c1 << 4) | (c2 >> 2); }
        if (src[i+3] != '=') { if ((signed char)c3 < 0) return MBEDTLS_ERR_BASE64_INVALID_CHARACTER; dst[n++] = (c2 << 6) | c3; }
    }
    *olen = n;
    return 0;
}
